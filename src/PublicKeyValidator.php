...
class PublicKeyValidator
{
    ...
    public function parseSignatureBlob(string $signatureBlob): array|false
    {
        try {
            ...
            if ($algorithmLength <= 0 || $algorithmLength > $blobLength - 4) {
                $this->logger->error("Invalid algorithm length: {$algorithmLength}");
                return false;
            }
            ...
            if ($offset + 4 > $blobLength) {
                $this->logger->error("Signature blob too short for signature length");
                return false;
            }
            ...
            if ($signatureLength <= 0 || $offset + $signatureLength > $blobLength) {
                $this->logger->error("Invalid signature length: {$signatureLength}");
                return false;
            }
            ...
            if ($remaining > 0) {
                $this->logger->warning("Extra data in signature blob: {$remaining} bytes remaining");
            }

            return [$algorithm, $signature];
        } catch (\Exception $e) {
            $this->logger->error("Failed to parse signature blob: " . $e->getMessage());
            return false;
        }
    }
    ...
    public function validateSignature(
        ...
        try {
            ...
            if (!$this->validateKeyBlob($keyBlob)) {
                $this->logger->error("Key blob validation failed");
                return false;
            }
            ...
            if (!$this->isSignatureAlgorithmCompatible($actualKeyAlgorithm, $signatureAlgorithm)) {
                $this->logger->warning("Incompatible key and signature algorithms: $actualKeyAlgorithm / $signatureAlgorithm");
                return false;
            }
            ...
            $this->logger->error("Signature validation failed: " . $e->getMessage());
    ...
    private function isSignatureAlgorithmCompatible(string $keyAlgorithm, string $sigAlgorithm): bool
    {
        ...
        if (!$result) {
            $this->logger->warning("Incompatible combination: key algorithm '{$keyAlgorithm}' with signature algorithm '{$sigAlgorithm}'");

            if (in_array($keyAlgorithm, ['rsa-sha2-256', 'rsa-sha2-512'])) {
                $this->logger->warning("'{$keyAlgorithm}' is a signature algorithm, not a key type. Expected key type for RSA is 'ssh-rsa'.");
            }
        }
    ...
    private function verifyEd25519Signature(string $publicKeyBlob, string $signatureData, string $signedData, string $sigAlgorithm): bool
    {
        try {
            ...
            if ($publicKey === false) {
                $this->logger->error("Failed to extract Ed25519 public key from blob");
                return false;
            }

            // Verify signature length
            if (strlen($signatureData) !== SODIUM_CRYPTO_SIGN_BYTES) {
                $this->logger->error("Invalid Ed25519 signature length: got " . strlen($signatureData) .
            ...
            $this->logger->error("Ed25519 signature verification exception: " . $e->getMessage());
            $this->logger->error("Stack trace: " . $e->getTraceAsString());
            return false;
        }
    }
    ...
    public function verifyRsaSignature(string $publicKeyData, string $signatureData, string $signedData, string $sigAlgorithm): bool
    {
        try {
            ...
            if ($components === false) {
                $this->logger->error("Failed to extract RSA components from key blob");
                return false;
            }
            ...
            if ($publicKeyPem === false) {
                $this->logger->error("Failed to create PEM from RSA components");
                return false;
            }
            ...
            if ($opensslAlgorithm === null) {
                $this->logger->error("Unsupported signature algorithm: {$sigAlgorithm}");
                return false;
            }
            ...
            if ($publicKey === false) {
                $this->logger->error("Failed to load PEM key. OpenSSL Error: " . openssl_error_string());
                return false;
            }
            ...
            if ($keyDetails === false) {
                $this->logger->error("Failed to get key details. OpenSSL Error: " . openssl_error_string());
                return false;
            }
            ...
            if ($result === 1) {
                ...
                $this->logger->warning("RSA signature verification failed - invalid signature");
                return false;
            } else {
                $this->logger->error("RSA signature verification error. OpenSSL Error: " . openssl_error_string());
                return false;
            }
        } catch (\Exception $e) {
            $this->logger->error("RSA signature verification exception: " . $e->getMessage());
            $this->logger->error("Stack trace: " . $e->getTraceAsString());
            return false;
        }
    }
    ...
    protected function extractRsaComponents(string $keyBlob): array|false
    {
        try {
            ...
            if ($keyType !== 'ssh-rsa') {
                $this->logger->error("Invalid key type: {$keyType}, expected ssh-rsa");
                return false;
            }
            ...
            $this->logger->error("Failed to extract RSA components: " . $e->getMessage());
            return false;
        }
    }
    ...
    protected function createPemFromComponents(string $modulus, string $exponent): string|false
    {
        try {
            ...
            $this->logger->error("Failed to create PEM from components: " . $e->getMessage());
            return false;
        }
    }
    ...
    public function validateKeyBlob(string $keyBlob): bool {
        try {
            ...
            if ($keyType === 'ssh-rsa') {
                ...
                if ($pem === false) {
                    $this->logger->error("Failed to create PEM from key components");
                    return false;
                }
                ...
                if (strlen($publicKey) !== SODIUM_CRYPTO_SIGN_PUBLICKEYBYTES) {
                    $this->logger->error("Invalid Ed25519 public key length: " . strlen($publicKey) .
                ...
                if ($offset < strlen($keyBlob)) {
                    $this->logger->warning("Extra data in Ed25519 key blob: " .
            ...
            $this->logger->error("Key blob validation failed: " . $e->getMessage());
            return false;
        }
    }
    ...
    protected function extractEd25519PublicKey(string $keyBlob): string|false {
        try {
            ...
            if ($keyType !== 'ssh-ed25519') {
                $this->logger->error("Invalid key type: {$keyType}, expected ssh-ed25519");
                return false;
            }
            ...
            [$publicKey, $offset] = $this->extractString($keyBlob, $offset);
            if (strlen($publicKey) !== SODIUM_CRYPTO_SIGN_PUBLICKEYBYTES) {
                $this->logger->error("Invalid Ed25519 public key length: got " . strlen($publicKey) .
            ...
            $this->logger->error("Failed to extract Ed25519 public key: " . $e->getMessage());
            return false;
        }
    }
